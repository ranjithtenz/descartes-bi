...
#					self.fields[f.filter.name] = forms.DateField(('%d/%m/%Y'),initial=f.filter.default, required=False, label=f.filter.label, input_formats=['%m/%d/%Y'], widget=forms.DateInput(attrs={'size':'10'}))
					self.fields[f.filter.name] = forms.DateField(('%m/%d/%Y',),initial=f.filter.default, required=False, label=f.filter.label, widget=forms.DateInput(format='%m/%d/%Y',attrs={'size':'10'}))
					...
					self.fields[f.filter.name] = forms.ChoiceField(initial=f.filter.default, required=False, label=f.filter.label, choices = choices)
				elif f.filter.type == FILTER_TYPE_MONTH:
					self.fields[f.filter.name] = forms.ChoiceField(initial=f.filter.default, required=False, label=f.filter.label, choices = (('1',_(u'January')),('2', _(u'February')),('3', _(u'March')),('4', _(u'April')),('5', _(u'May')),('6', _(u'June')),('7', _(u'July')),('8', _(u'August')),('9', _(u'September')),('10', _(u'October')),('11', _(u'November')),('12', _(u'December'))))

		self.fields['output_type'] = forms.ChoiceField(initial='chart', required=False, label=_(u'Format'), choices = (('chart', _(u'Chart')),('grid', _(u'Table'))))
	class Media:
		js = ('js/filterform.js',)
