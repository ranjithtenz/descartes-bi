...
class get_menu_node(template.Node):
	def render(self, context):
		...
		mi_order.sort()
		context['menuitems_allowed'] = [Menuitem.objects.get(order=order) for order in mi_order]
		...
		for app in custom_apps:
			...
			try:
				...
				if 'template_name' in custom_apps_data[app]:
					template = get_template(custom_apps_data[app]['template_name'])
					custom_apps_data[app]['template_data'] = template.render(Context(context))
			except:
				pass
