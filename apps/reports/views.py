...
def ajax_filter_form(request, report_id):
	...
	if report not in _get_allowed_object_for_user(request.user)['reports']:
		return render_to_response('messagebox-error.html', { 'title' : _(u'Permission error'), 'message': _(u"Insufficient permissions to access this area.") })		
...
def ajax_report_validation(request, report_id):                
	...
	for s in report.series.all():
		if s.validated:
			...
			result += "<li>'%s' validated on %s" % (s.label or unicode(s), s.validated_date)
	...
	if orientation == 'v':
		...
		for key,value in data:
			try:
				...
				result += '[%s,"%s","%s"],' % (value, unicode(key or '?'), unicode(value))  
